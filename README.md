...
در حال توسعه (Phase 2 — Auth, Domain Models, Cart & Orders)

بک‌اند NestJS با دیتابیس PostgreSQL واقعی، مدل‌های دامنه (Product, Category, User, Cart, Order) با روابط واقعی بین آن‌ها، سیستم احراز هویت کامل (JWT)، سبد خرید، و ثبت سفارش تراکنشی پیاده‌سازی شده. فرانت‌اند هنوز نسخه‌ی استاتیک اولیه است (در docs/legacy-prototype نگهداری می‌شود).
...
| POST | /orders | ثبت سفارش از روی سبد فعلی (تراکنشی) | بله (JWT) |
| GET | /orders | تاریخچه‌ی سفارش‌های کاربر | بله (JWT) |
...
- [x] ثبت سفارش تراکنشی با snapshot قیمت (Order & OrderItem)
- [ ] پنل مدیریت
